...
// full INVERSE is applied on export so files leave Texturizador in their original
...
console.info(`Texturizador v${APP_VERSION}`);
...
document.getElementById('theme-toggle').addEventListener('click', () => {
  ...
  localStorage.setItem('texturizador-theme', isLight ? 'light' : 'dark');
...
loadAllThumbnails().then(thumbs => {
  ...
  try {
    const raw = sessionStorage.getItem('texturizador-settings');
...
async function handleExport(format = 'stl') {
  ...
  try {
    ...
    if (result.repairStats) {
      ...
      console.log(
        `%c[Texturizador] mesh repair (build 2026-06-10w): ` +
  ...
  if (!_pipelineWorkerInit) {
    _pipelineWorkerInit = _initPipelineWorker().then(
      ...
      (err) => {
        ...
        console.warn('[Texturizador] export worker unavailable — running pipeline on the main thread:', err.message);
        return null;
      }
    );
  }
...
// ── Project save/load (.texturizador) + sessionStorage auto-save ────────────────
// .texturizador is a ZIP containing: settings.json (required), model.stl (optional),
...
const PROJECT_STORAGE_KEY = 'texturizador-settings';
...
// ── Export: build .texturizador ZIP and trigger download ─────────────────────────
...
exportGoBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    _downloadBlob(new Blob([zipped], { type: 'application/octet-stream' }),
                  (currentStlName || 'proyecto') + '.texturizador');
  } catch (err) {
    alert(t('alerts.exportFailed', { msg: err.message }));
  }
});
 ...
 * that would be out-of-range for the loaded mesh (defensive — the .texturizador
...
async function importProject(file) {
  ...
  // A .texturizador project is a ZIP (PK\x03\x04). If the user picked a bare model
  // file here instead — common on macOS Chrome, where the accept=".texturizador"
